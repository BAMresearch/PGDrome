...
class PGDProblem:
    def __init__(
        ...
        **kwargs,
        ...
        self.modes_info = (
            modes_info  # list with infos for pxdmf file e.g. ['U_x', 'Node', 'Scalar']
        )
        ...
        self.bc_fct = (
            bc_fct  # function which gives list of boundaries (use as input Vs,dom)
        )
        ...
        self.prob = (
            probs  # list of problem strings e.g. ['r','s'] used in assemble fcts
        )
        if len(seq_fp) == 0:
        ...
        self.tol_abs = (
            1e-6  # absolute tolerance of fixed point iteration (norm criterion)
        )
        ...
        self.simulation_info = (
            "PGD solver option: PGD_nmax %s / PGD tolerance %s and max FP iterations %s and FP tolerance %s; \n"
    ...
    def bc(self):
        ...
        dom_tmp = self.dom
        self.logger.debug(
            "boundary array: %s", self.bc_fct(self.V, dom_tmp, self.param)
        )
            ...
            if check_string.split(" ")[0] == "<vector":
                ...
                if dimension_dim == 1:  # scalar Function Space
                    ...
                    if self.fp_init.lower() == "randomized":  # as in Chadys matlab code
                        idx = np.where(Fs_init[dim].vector()[:] != 0)[
                            0
                        ]  # idx without boundary
                        Fs_init[dim].vector()[idx] = np.random.rand(len(idx))

                    # normalize
                    if solve_modes != None and solve_modes[dim] == "FD":
                        norm_Fs_init = np.sqrt(
                            Fs_init[dim].vector()[:].transpose()
                            @ self.MM[dim]
                            @ Fs_init[dim].vector()[:]
                        )  # norm with given M matrix
                    else:
                        norm_Fs_init = dolfin.norm(Fs_init[dim])  # classical l2 norm
                    Fs_init[dim].vector()[:] *= 1.0 / norm_Fs_init  # normalization
                    self.logger.debug(
                        "Fs_init[dim]: %s ", Fs_init[dim].compute_vertex_values()[:]
                    )

                elif dimension_dim == 2:  # VectorFunctionSpace!!!
                    Fs_init[dim] = dolfin.interpolate(
                        dolfin.Expression(("1.0", "1.0"), element=V[dim].ufl_element()),
                        V=V[dim],
                    )  # , solver_type='mumps')
                    if bc[dim] != 0:  # apply boundary conditions
                        if isinstance(bc[dim], list):
                    ...
                    Fs_init[dim].vector()[:] *= 1.0 / dolfin.norm(
                        Fs_init[dim]
                    )  # normalization with l2 norm (no FD option here just for 1D)
                elif dimension_dim == 3:  # VectorFunctionSpace!!!
                    Fs_init[dim] = dolfin.interpolate(
                        dolfin.Expression(
                            ("1.0", "1.0", "1.0"), element=V[dim].ufl_element()
                        ),
                        V=V[dim],
                    )  # , solver_type='mumps')
                    ...
                    Fs_init[dim].vector()[:] *= 1.0 / dolfin.norm(
                        Fs_init[dim]
                    )  # normalization with l2 norm (no FD options here just in 1D)
                else:
                ...
                self.logger.debug(
                    "Fs_init[dim] before normalization: %s",
                    Fs_init[dim].compute_vertex_values()[:],
                )
                Fs_init[dim].vector()[:] *= (
                    1.0 / norm_Fs_init
    ...
    def solve_PGD(
        ...
        while n_enr < self.PGD_nmax - 1:
            ...
            # FP iteration
            Fs, norm_Fs = self.FP_solve(
                Fs_init, norm_Fs, delta, n_enr, _problem, solve_modes, settings
            )
            ...
            if self.norm_modes.lower() == "no":
                ...
                if solve_modes != None and solve_modes[-1] == self.solve_mode["FD"]:
                    norm_aux = (
                        var_F.vector()[:].transpose() @ a @ fct_F.vector()[:]
                    )  # a== matrix
                elif (
                    solve_modes != None and solve_modes[-1] == self.solve_mode["direct"]
                ):
                    norm_aux = a  # scalar
                else:
                    norm_aux = dolfin.assemble(a)  # a== form
                norm_fac = np.sqrt(np.absolute(norm_aux)) ** (1.0 / self.num_pgd_var)
                self.alpha.append(
                    np.prod(norm_Fs) * norm_fac**self.num_pgd_var
                )  # joined norm factor

                for dim in range(self.num_pgd_var):
                    Fs_normalized[dim].vector()[:] *= 1.0 / norm_fac
                    Fs_normalized[dim].vector()[:] *= self.alpha[-1] ** (
                        1.0 / self.num_pgd_var
                    )  # multiply each mode by a part of the joined norm
                ...
                norm_all = np.prod(norm_Fs) ** (1.0 / self.num_pgd_var)
                for dim in range(self.num_pgd_var):
            ...
            self.logger.info(
                "PGD modes updated: normU=%s; relNorm=%s; tol=%s; res_error=%s",
                normU,
                relConv[n_enr],
                self.PGD_tol,
                res_error,
    ...
    def FP_solve(self, Fs_init, norm_Fs, delta, n_enr, _problem, solve_modes, settings):
        ...
        ffc_options = {
            "optimize": True
        }  # parameters are set globally in parameters["form_compiler"]
        ...
        for fpi in range(self.max_fp_it):
            ...
            for seq in range(len(self.seq_fp)):
                ...
                if self.bc[dim] == 0:
                    ...
                    if (
                        solve_modes is None
                        or solve_modes[dim] == self.solve_mode["FEM"]
                    ):  # standard FEM solver
                        if _problem.lower() == "nonlinear":
                            ...
                            problem = dolfin.NonlinearVariationalProblem(
                                a - l, fct_F, J=J, form_compiler_parameters=ffc_options
                            )
                            ...
                            fct_F = dolfin.Function(self.V[dim])
                            problem = dolfin.LinearVariationalProblem(
                                a, l, fct_F, form_compiler_parameters=ffc_options
                            )
                        ...
                        self.logger.error(
                            "ERROR: solver %s doesn't exist", solve_modes[dim]
                        )
                else:
                ...
                self.logger.debug(
                    "problem %s: F_%s_max: %s",
                    self.prob[dim],
                    str(dim),
                    np.absolute(fct_F.vector()[:]).max(),
                )
                self.logger.debug(
                    "problem %s: Finit_%s: %s =? %s",
                    self.prob[dim],
                    str(dim),
                    Fs_init[dim].vector()[:],
                    Fs[dim].vector()[:],
            ...
            self.logger.debug(
                "convergence type for fixed point iteration: %s (changeable in self.stop_fp)",
                self.stop_fp,
            )
            ...
            if self.stop_fp.lower() == "delta":
                for dim in range(self.num_pgd_var):
                    ...
                    delta_tmp = np.absolute(
                        Fs[dim].vector()[:] - Fs_init[dim].vector()[:]
                    )
                    max_index = np.argmax(delta_tmp)
                    if np.absolute(Fs[dim].vector()[max_index]) < 1e-8:
                        delta[dim] = delta_tmp.max()
                    else:
                        delta[dim] = delta_tmp.max() / np.absolute(
                            Fs[dim].vector()[max_index]
                        )  # relative
                self.logger.debug(f"absolute delta {delta_tmp}, relative delta {delta}")

                if (
                    len(np.where(delta > self.tol_fp_it)[0]) > 0
                    and fpi < self.max_fp_it - 1
                ):
                    self.logger.debug(
                        "fix point iteration not converged %s (max %s)",
                        fpi,
                        self.max_fp_it,
                    )
                    Fs_init = np.copy(Fs)
                    ...
                    self.logger.info(
                        "fix point iteration converged !!! in number of steps: %s (delta:%s)",
                        fpi + 1,
                        delta,
                    )
                ...
                for d in range(self.num_pgd_var):
                    if solve_modes != None and solve_modes[d] == "FD":
                        ...
                        newnew *= (
                            dolfin.norm(Fs[d]) ** 2
                        )  # same as dolfin.assemble(Fs[d]*Fs[d]* dolfin.dx(self.meshes[d]))
                        newold *= dolfin.assemble(
                            dolfin.inner(Fs[d], Fs_init[d]) * dolfin.dx(self.meshes[d])
                        )
                ...
                if max_error < self.tol_fp_it:
                    ...
                    self.logger.debug(
                        "fix point iteration not converged %s (max %s) (error %s)",
                        fpi,
                        self.max_fp_it,
                        max_error,
                    )
                ...
                self.logger.error(
                    'stopping criterion not defined %s (self.stop_fp = "delta" or "norm")',
                    self.stop_fp,
                )
                raise ValueError(
                    'stopping criterion not defined %s (self.stop_fp = "delta" or "norm")'
    ...
    def return_PGD(self):
        ...
        solution = PGD(
            name=self.name,
            n_modes=self.PGD_modes,
            fmeshes=self.meshes,
            pgd_modes=self.PGD_func,
