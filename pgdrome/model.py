...
class PGD:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.folder = (
            ""  # string to folder where the pxdmf file as well as xdmf and h5 files are
        )
        ...
        self._eval_fixed_modes = (
            {}
        )  # accessed with self.eval_fixed_modes which is cached

    def __str__(self):
        return "PGD(name: %s)(meshes: %s)(modes: %s)" % (
            self.name,
            len(self.mesh),
            self.numModes,
        )
    ...
    def eval_fixed_modes(self, sensor_points, fixed_dim, attri):
        try:
            ...
            LOGGER.error(
                f"Failed loading fenicstools with error: {e}, required for function all sensor evaluations for speed-up"
            )
        ...
        probes = Probes(
            sensor_points.flatten(),
            self.mesh[fixed_dim].attributes[attri].interpolationfct[0].function_space(),
        )
        for k in range(
            self.numModes
        ):  # chached for ALL numModes in evaluate only used_numModes will be computed!!
        ...
        for coord, mesh in enumerate(self.mesh):
            ...
            for att in mesh.attributes:
                for mode in range(self.numModes):
                    ...
                    self.logger.debug(
                        "Functionspace degree: %s ",
                        att.interpolationfct[mode].ufl_element().degree(),
                    )
            file_out.close()
    ...
    def write_pxdmf(self, folder, xdmf_exist=False):
        ...
        with open(os.path.join(folder, self.name + ".pxdmf"), "w") as file_out:
            ...
            for cur_mesh in self.mesh:
                ...
                # informations
                file_out.write(
                    '      <Information Name="Dims" Value="'
                    + str(cur_mesh.info[0])
                    + '" />\n'
                )
                file_out.write(
                    '      <Information Name="Dim0" Value="'
                    + cur_mesh.info[1]
                    + '" />\n'
                )
                file_out.write(
                    '      <Information Name="Unit0" Value="'
                    + cur_mesh.info[2]
                    + '" />\n'
                )
                ...
                file_out.write(
                    ...
                    + str(cur_mesh.numElements)
                    + '" TopologyType = "'
                    + cur_mesh.typElements
                    + '" NodesPerElement = "'
                    + str(nodes_per_element)
                    + '" >\n'
                )
                file_out.write(
                    '          <DataItem Dimensions = "'
                    ...
                    + cur_mesh.name
                    + ".h5:/Mesh/0/mesh/topology</DataItem>\n"
                )
                ...
                if np.array(groupg).shape[1] == 2:
                    ...
                    file_out.write(
                        '          <DataItem Dimensions = "'
                        + str(np.array(groupg).shape[0])
                        + " "
                        + str(np.array(groupg).shape[1])
                        + '" Format = "HDF">'
                        + cur_mesh.name
                        + ".h5:/Mesh/0/mesh/geometry</DataItem>\n"
                    )
                ...
                for cur_attr in cur_mesh.attributes:
                    for count in range(len(cur_attr.data)):
                        if (
                            cur_attr.field.lower() == "vector" and xdmf_exist
                        ):  # check if we need to extend attribute data
                            # extend data to 3D PGD2.h5:/VisualisationVector/0
                            data_hdf = np.array(
                                hf.get("/VisualisationVector/" + str(count))
                            )
                            ...
                            file_out.write(
                                '          <DataItem Dimensions="'
                                + str(cur_attr.data[count].shape[0])
                                + " "
                                + str(3)
                                + '" Format="XML" NumberType="float" >\n'
                            )
                            ...
                            file_out.write(
                                ...
                                + str(cur_attr.data[count].shape[0])
                                + " "
                                + str(cur_attr.data[count].shape[1])
                                + '" Format="HDF">'
                                + cur_mesh.name
                                + ".h5:/VisualisationVector/"
                                + str(count)
                                + "</DataItem>\n"
                            )
                            file_out.write("        </Attribute>\n")
    ...
    def load_pxdmf(self, filepath, verbose=False):
        ...
        for PGDs in xmlroot.iter("Grid"):
            ...
            for elems in PGDs.iter("Information"):
                pgd_mesh.info.append(
                    [elems.attrib.get("Name"), elems.attrib.get("Value")]
                )
            ...
            for elems in PGDs.iter("Topology"):
                ...
                if elems[0].get("Format") == "XML":
                    ...
                    topo = data_to_array(
                        elems[0].text, "int"
                    )  # read data and save them into a list integer
                    ...
                    with h5py.File(
                        folder + "/" + elems[0].text.split(":")[0], "r"
                    ) as hf:
                        # hf = h5py.File(folder + '/' + elems[0].text.split(':')[0], 'r')
                        pgd_mesh.topology = np.array(
                            hf.get(elems[0].text.split(":")[1])
                        )
            ...
            for elems in PGDs.iter("Geometry"):
                if elems[0].get("Format") == "XML":
                    ...
                    geom = np.array(
                        data_to_array(elems[0].text, "float")
                    )  # read data and save them into a list integer
            ...
            for elems in PGDs.iter("Attribute"):
                ...
                if check:
                    ...
                    if elems[0].get("Format") == "XML":
                        pgd_mesh.attributes[position].data.append(
                            np.array(data_to_array(elems[0].text, "float"))
                        )
                    elif elems[0].get("Format") == "HDF":
                        with h5py.File(
                            folder + "/" + elems[0].text.split(":")[0], "r"
                            ...
                            pgd_mesh.attributes[position].data.append(
                                np.array(hf.get(elems[0].text.split(":")[1]))
                            )
        ...
        self.numModes = len(
            self.mesh[0].attributes[0].data
        )  # have do be the same for each mesh and attribute!!
    ...
    def create_interpolation_fcts(self, free_dim, attri, verbose=True):
        ...
        if len(free_dim) > self.num_pgd_var:
            raise ValueError(
                "given number of Dimensions larger then existing Meshes in PGD solution"
            )
        ...
        for i in range(len(free_dim)):  # loop over free Dimensions
            ...
            if self.mesh[free_dim[i]].attributes[attri].interpolationInfo["name"] == 0:
                ...
                if (
                    ...
                    raise ValueError(
                        "free Dimensions are not 1D, interpolation with INTERP1D not possible"
                    )
                else:
                    self.logger.debug(
                        "interp1d from scipy will be used for interpolation"
                    )
                    string = "scipy.interpol1d"
                    str_kind = (
                        self.mesh[free_dim[i]]
                        .attributes[attri]
                        .interpolationInfo["kind"]
                    )
                    for k in range(self.numModes):  # loop over PGD modes
                        ...
                        self.mesh[free_dim[i]].attributes[
                            attri
                        ].interpolationfct.append(interfct)

            elif (
                self.mesh[free_dim[i]].attributes[attri].interpolationInfo["name"] == 1
            ):
                self.logger.debug(
                ...
                if verbose:
                    print(
                        "test",
                        self.mesh[free_dim[i]].attributes[attri].interpolationInfo,
                    )
                str_family = (
                    self.mesh[free_dim[i]].attributes[attri].interpolationInfo["family"]
                )
                int_degree = int(
                    self.mesh[free_dim[i]].attributes[attri].interpolationInfo["degree"]
                )
                str_fs_type = (
                    self.mesh[free_dim[i]].attributes[attri].interpolationInfo["_type"]
                )

                filepath = os.path.abspath(
                    os.path.join(self.folder, self.mesh[free_dim[i]].name + "_data.h5")
                )
                ...
                self.logger.debug(
                    ...
                    free_dim[i],
                    self.mesh[free_dim[i]].attributes[attri].field.lower(),
                    str_fs_type,
                )
                if str_fs_type.lower() == "scalar":
                    ...
                    Vspace = dolfin.VectorFunctionSpace(
                        mesh_hdf, str_family, int_degree
                    )
                else:
                    raise ValueError(
                        "function space type not defined or wrong defined %s"
                        % (str_fs_type)
                    )
                ...
                for k in range(self.numModes):
                    ...
                    self.mesh[free_dim[i]].attributes[attri].interpolationfct.append(
                        f_mode
                    )
                self.logger.debug(
                    "created function with following function space: %s",
                ...
                self.logger.error(
                    "interpolation name not defined: %s",
    ...
    def evaluate(self, fixed_dim, free_dim, coord, attri):
        ...
        if len(free_dim) != self.num_pgd_var - 1:
            raise ValueError(
                "given variables are missing or to much, free_dim=%s <-> num_pgd_var=%s",
                free_dim,
                self.num_pgd_var - 1,
            )
        ...
        if len(coord) != self.num_pgd_var - 1:
            raise ValueError(
                "given variables are missing or to much, coord=%s <-> num_pgd_var=%s",
                coord,
                self.num_pgd_var - 1,
            )
        ...
        if len(free_dim) != len(coord):
            raise ValueError(
                "Number of free Dimensions and given coordinates are not the same, free_dim=%s <-> coord=%s",
                free_dim,
                coord,
            )
        ...
        if (
            self.mesh[free_dim[0]].attributes[attri].interpolationInfo["name"] == 0
        ):  # interp1 function
            # initialize shape of PGD mode
            # same dimension as in data (scalar n x 1 or vector n x 2or3)
            eval = np.zeros(
                self.mesh[fixed_dim].attributes[attri].data[0].shape
            )  # nodal or cell values depending on attribute mode_info!!!
            for k in range(
                self.used_numModes
            ):  # normally used_numModes == numModes - for easy change
    ...
    def evaluate_sensor_response(
        self, fixed_dim, free_dim, coord, attri, sensor_points
    ):
        """
        Reconstruct pgd solution for the fixed variable where all
        ...
        self.logger.debug(
            "shape of eval_fixedmode is %s (len %s)",
            eval_fixedmode.shape,
            len(eval_fixedmode.shape),
        )
        ...
        if self.numModes == 1:
            ...
            if len(eval_fixedmode.shape) == 3:  # Vectorfield
                eval_array = np.sum(
                    eval_fixedmode[:, :, 0 : self.used_numModes] * tmp, axis=2
                )
            elif len(eval_fixedmode.shape) == 2:  # Scalarfield
                eval_array = np.sum(
                    eval_fixedmode[:, 0 : self.used_numModes] * tmp, axis=1
                )
    ...
    def evaluate_min_abs(self, fixed_dim, free_dim, coord, attri, *args, **kwargs):
        ...
        if (
            ...
            return abs(
                self.evaluate(fixed_dim, free_dim, coord, attri).vector()[:]
            ).min()
    ...
    def create_derivation_fct(self, free_dim, attri):
        ...
        for i in range(len(free_dim)):  # loop over free Dimensions
            ...
            if self.mesh[free_dim[i]].attributes[attri].interpolationInfo["name"] == 0:
                raise ValueError(
                    "derivation for interp1 functions not implemented (only fencis functions)"
                )
                ...
                self.logger.debug(
                    "Functionspace of dim %s is from type %s", free_dim[i], check_string
                )
                if check_string.split(" ")[0] == "<vector":
                    Vspace = dolfin.TensorFunctionSpace(
                        self.mesh[free_dim[i]].fenics_mesh, "DG", int_degree
                    )
                    for k in range(self.numModes):
                        f_mode = (
                            self.mesh[free_dim[i]].attributes[attri].interpolationfct[k]
                        )
                        d_mode = dolfin.project(dolfin.grad(f_mode), Vspace)
                        self.mesh[free_dim[i]].attributes[attri].derivationfct.append(
                            d_mode
                        )
                    self.logger.info(
                        "derivation from vector valued function --> tensor --> NOT YET TESTED!!!"
                    )
                    self.logger.debug(
                        "Functionspace of dim %s is from type %s",
                        free_dim[i],
                        check_string,
                    )
                    self.logger.debug(
                        "Derivation-Functionspace of dim %s is from type %s",
                        free_dim[i],
                        d_mode.function_space().ufl_function_space().ufl_element(),
                    )
                else:
                    Vspace = dolfin.FunctionSpace(
                        self.mesh[free_dim[i]].fenics_mesh, "DG", int_degree
                    )
        ...
        self.logger.info(
            "derivations for dimensions %s are saved in PGD instance" % (free_dim)
        )
            ...
            for k in range(
                ...
                for i in range(len(free_dim)):  # loop over free Dimensions
                    if free_dim[i] == d_dim:
                        ...
                        self.logger.debug(
                            "use derivation from d_dim %s mode number %s", d_dim, k
                        )
                        faci = (
                            self.mesh[free_dim[i]]
                            .attributes[attri]
                            .derivationfct[k](coord[i])
                        )
                    else:
                        # coord needs right dimension scalar or vector
                        faci = (
    ...
    def evaluate_derivative_sensor_response(
        self, fixed_dim, free_dim, coord, attri, d_dim, sensor_points
    ):
        """
        Reconstruct derivation of pgd solution against d_dim for the fixed variable where all
    ...
    def save_modes_latex(self, folder, attri, prefix="_"):
        ...
        for k in range(self.num_pgd_var):
            if self.mesh[k].typElements.lower() == "polyline":
                ...
                dof_number = len(
                    self.mesh[k].attributes[attri].interpolationfct[0].vector()[:]
                )
                ...
                for m in range(self.numModes):
                    zip_data = zip(
                        dof_coord,
                        self.mesh[k].attributes[attri].interpolationfct[m].vector()[:],
                    )
                ...
                np.savetxt(
                    os.path.join(
                        folder, name % (prefix, attri, self.mesh[k].info[1][1])
                    ),
                    out_k,
                    delimiter=",",
                )
...
class PGDAttribute(object):
    def __init__(self, num_modes=0, mesh=None, pgd_modes=None, modes_info=None):
        ...
        if modes_info is not None:
            ...
            self.field = modes_info[
                2
            ]  # scalar or vector field for overall variable (NOT FOR PGD variables!!)
        self.data = (
            list()
        )  # PGD modes [Number of values (Nodes or Cell) x 1 (Skalarfield) or 3 (Vectorfield)] x PGDsolution.numModes]

        self.interpolationInfo = {
            "name": 1
        }  # what kind of interpolation method should be used
        ...
        for ctr in range(num_modes):
            self.interpolationfct.append(
                pgd_modes[ctr]
            )  # fenics function space functions of PGD modes
    ...
    def fill_data(self, num_modes, mesh, pgd_modes):
        ...
        self.logger.debug(
            "fill_data for attribute based on fenics function and mode_info[1] (Node or Cell) and mode_info[2] (scalar or vector)"
        )
        self.data = list()
        for ctr in range(num_modes):
            if self._type.lower() == "node":
                ...
                raise ValueError(
                    " Error in filling attribute data: self._type not known"
                )
...
class PGDMesh(object):
    ...
    def __init__(
        ...
        self.meshdim = (
            mesh.topology().dim() if mesh is not None else 0
        )  # mesh dimension known from fenics mesh!!
        self.info = [
            self.meshdim,
            name_coord,
            "-?-",
        ]  # list contains Dimension, Name and Metrics
        self.numElements = mesh.num_cells() if mesh is not None else None
        self.numNodes = mesh.num_vertices() if mesh is not None else 0
        self.topology = (
            mesh.cells() if mesh is not None else None
        )  # Mesh data: element point connection [Number of elements x points per element]
        self.typGeometry = "XYZ"  # Typ of Geometry: default XYZ
        self.dataX = np.zeros(
            self.numNodes
        )  # xyz data of points [Number of points x 1]
...
class PGDErrorComputation(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.free_dim = [
            item
            for item in list(range(0, self.PGD_sol.num_pgd_var))
            if item not in fixed_dim
        ]
        ...
        if not self.lim_smp:
            ...
            for i in self.free_dim:
                if len(self.PGD_sol.problem.meshes[i].coordinates()[0]) == 1:
                    ...
                    max_bnd[ind] = float(
                        max(self.PGD_sol.problem.meshes[i].coordinates())
                    )  # Maximum boundary
                    ind = ind + 1
                else:
    ...
    def compute_SampleError(self, u_FOM, u_PGD):
        ...
        if isinstance(u_FOM, np.ndarray) and isinstance(u_PGD, np.ndarray):
            ...
            error = dolfin.norm(u_FOM.vector() - u_PGD.vector(), "l2") / dolfin.norm(
                u_FOM.vector(), "l2"
            )
    ...
    def evaluate_error(self):
        ...
        for i in range(len(self.data_test)):
            ...
            if self.PGD_sol:
                ...
                u_pgd = self.PGD_sol.evaluate(
                    int(self.fixed_dim[0]), self.free_dim, self.data_test[i], 0
                )
