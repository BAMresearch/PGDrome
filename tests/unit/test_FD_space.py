...
class FEM_solution_dirichlet:
    ...
    def run(self):
        ...
        bc = [dolfin.DirichletBC(self.Vs, 100, left),
...
class FEM_solution_robin:
    ...
    def run(self):
        ...
        boundary_parts = fenics.MeshFunction("size_t", self.meshes, self.meshes.topology().dim() - 1)
        ...
        T_amb_func.vector()[:] = self.param['T_amb'] * np.ones(len(T_amb_func.vector()[:]))

        F = fenics.dot(fenics.grad(T), fenics.grad(v)) * fenics.dx() \
            - self.q * v * fenics.dx() \
...
class PGDproblem(unittest.TestCase):
    ...
    def test_solver(self):
        ...
        TFD_dirichlet = FD_solution_dirichlet(Vs=Vs_x, meshes=mesh_x, param=self.param, q=q).run()
        # print(TFD_dirichlet.compute_vertex_values()[:])

        TFD_robin = FD_solution_robin(Vs=Vs_x, meshes=mesh_x, param=self.param, q=q).run()
        # print(TFD_robin.compute_vertex_values()[:])

        TFEM_dirichlet = FEM_solution_dirichlet(Vs=Vs_x, meshes=mesh_x, param=self.param, q=q).run()
        # print(TFEM_dirichlet.compute_vertex_values()[:])

        TFEM_robin = FEM_solution_robin(Vs=Vs_x, meshes=mesh_x, param=self.param, q=q).run()
        ...
        self.assertTrue(error_robin < 1e-1)  # expect up to single percent error for robin bc
        ...
        if self.plotting:
            ...
            plt.plot(
                mesh_x.coordinates()[:],
                100 * (TFEM_robin.compute_vertex_values()[:] - TFD_robin.compute_vertex_values()[:]) /
                TFEM_robin.compute_vertex_values()[:],
                "-*b",
                label="error FD- FEM",
            )
