...
class u_analytic:
    ...
    def __call__(self, values):
        ...
        return (
            ...
            * 1
            / (self.param["A"] * E)
            * (self.param["lae"] * self.x - self.x**2)
            * L
            * self.param["n"]
        )
...
def create_example_pgd_solution(param):
    ...
    # create Coordinates
    Grid2.dataX = np.linspace(0.5, 1.0, num=Grid2.numNodes)
...
class TestPGD(unittest.TestCase):
    def setUp(self):
        ...
        # analytic solution
        self.u_ana = u_analytic(x=self.pgd.mesh[0].dataX, p=self.param)(
            [self.E, self.L]
        )
        self.sig_ana = sig_analytic(x=self.pgd.mesh[0].dataX, p=self.param)(
            [self.E, self.L]
        )
    ...
    def test_evaluate(self):
        ...
        for at in attri:
            ...
            self.pgd.mesh[1].attributes[at].interpolationInfo = {
                "name": 0,
                "kind": "linear",
            }
            self.pgd.mesh[2].attributes[at].interpolationInfo = {
                "name": 0,
                "kind": "linear",
            }
            # self.pgd.mesh[1].attributes[at].print_info()

            self.pgd.create_interpolation_fcts(
                [1, 2], at
            )  # not necessary but to test fct too
