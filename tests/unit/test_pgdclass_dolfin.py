...
class TestPGD(unittest.TestCase):
    def setUp(self):
        ...
        mesh_x = dolfin.IntervalMesh(50, 0.0, 1.0)
        V_x = dolfin.FunctionSpace(mesh_x, "CG", 1)

        mesh_p = dolfin.IntervalMesh(10, 0.0, 2.0)
        ...
        for nn in range(2):
            PGD_func[0].append(
                dolfin.project(dolfin.Expression("x[0]*x[0]", degree=10), V_x)
            )
            PGD_func[1].append(
                dolfin.project(dolfin.Expression("x[0]", degree=10), V_p)
            )
            PGD_func[2].append(
                dolfin.project(dolfin.Expression("1.0/x[0]", degree=10), V_E)
            )
    ...
    def test_load_and_evaluate(self):
        ...
        pgd_load.mesh[0].attributes[0].interpolationInfo = {
            "name": 1,
            "family": "CG",
            "degree": 1,
            "_type": "scalar",
        }
        pgd_load.mesh[1].attributes[0].interpolationInfo = {
            "name": 1,
            "family": "CG",
            "degree": 1,
            "_type": "scalar",
        }
        pgd_load.mesh[2].attributes[0].interpolationInfo = {
            "name": 1,
            "family": "CG",
            "degree": 2,
            "_type": "scalar",
        ...
        self.assertAlmostEqual(
            0.8**2, pgd_load.mesh[0].attributes[0].interpolationfct[0](0.8), places=3
        )
        self.assertAlmostEqual(
            0.8, pgd_load.mesh[1].attributes[0].interpolationfct[0](0.8), places=3
        )
        self.assertAlmostEqual(
            1 / 0.8, pgd_load.mesh[2].attributes[0].interpolationfct[0](0.8), places=3
        )
        # self.assertAlmostEqual()
        ...
        self.assertAlmostEqual(
            x_set**2 * self.P * 1 / self.E, evaluate(x_set), places=1
        )
