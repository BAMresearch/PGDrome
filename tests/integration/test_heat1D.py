...
class Reference:
    def __init__(self, param={}, vs=[], q=None, x_fixed=None):
        ...
        self.F = (
            self.param["rho"] * self.param["cp"] * T * v * dolfin.dx()
            + self.dt
            * self.param["k"]
            * dolfin.dot(dolfin.grad(T), dolfin.grad(v))
            * dolfin.dx()
            - (
                self.dt * self.Q * self.q
                + self.param["rho"] * self.param["cp"] * self.T_n
            )
            * v
            * dolfin.dx()
        )
...
class problem(unittest.TestCase):
    def setUp(self):
        ...
        self.ranges = [
            [0.0, self.param["lx"]],  # xmin, xmax
            [0.0, self.param["lt"]],  # tmin, tmax
            [0.5, 1.0],
        ]  # qmin, qmax
    ...
    def test_heating(self):
        ...
        self.param["Tamb_fct"] = dolfin.Expression(
            "Tamb", degree=1, Tamb=self.param["Tamb"]
        ...
        errors_FEM12 = np.linalg.norm(
            upgd_fem_bc - u_fem[tidx].compute_vertex_values()[:]
        ) / np.linalg.norm(
            u_fem[tidx].compute_vertex_values()[:]
        )  # PGD FEM - FEM
        ...
        upgd_fem2_bc = upgd_fem2.compute_vertex_values()[:] + param["IC_x"](
            self.x_fixed
        ...
        errors_FEM22 = np.linalg.norm(upgd_fem2_bc - u_fem2) / np.linalg.norm(
            u_fem2
        )  # PGD FEM - FEM
