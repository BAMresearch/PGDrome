...
def create_bc(Vs, dom, param):
    ...
    def leftright(x, on_boundary):
        return (
            on_boundary and df.near(x[0], 0.0, 1e-6) or df.near(x[0], param["lx"], 1e-6)
        )
...
def problem_assemble_lhs_FEM(fct_F, var_F, Fs, meshes, dom, param, typ, dim):
    ...
    if typ == "r":
        a = df.Constant(
        ...
        ) * param["k"] * fct_F.dx(0) * var_F.dx(0) * df.dx(meshes[0]) + df.Constant(
            ...
            * df.assemble(Fs[3] * Fs[3] * df.dx(meshes[3]))
            ...
            meshes[0]
        )  # k*dT/dx*DT/dx + k*dT/dx*DT/dx
    if typ == "s":
        a = df.Constant(
            df.assemble(Fs[0].dx(0) * Fs[0].dx(0) * df.dx(meshes[0]))
            * df.assemble(Fs[2] * Fs[2] * df.dx(meshes[2]))
            * df.assemble(Fs[3] * Fs[3] * df.dx(meshes[3]))
        ) * param["k"] * fct_F * var_F * df.dx(meshes[1]) + df.Constant(
            df.assemble(Fs[0] * Fs[0] * df.dx(meshes[0]))
            * df.assemble(Fs[2] * Fs[2] * df.dx(meshes[2]))
            * df.assemble(Fs[3] * Fs[3] * df.dx(meshes[3]))
        ) * param[
            "k"
    ...
    if typ == "t":
        a = df.Constant(
            ...
            * df.assemble(Fs[1] * Fs[1] * df.dx(meshes[1]))
            * df.assemble(Fs[3] * Fs[3] * df.dx(meshes[3]))
        ) * param["k"] * fct_F * var_F * df.dx(meshes[2]) + df.Constant(
            df.assemble(Fs[0] * Fs[0] * df.dx(meshes[0]))
            * df.assemble(Fs[1].dx(0) * Fs[1].dx(0) * df.dx(meshes[1]))
            * df.assemble(Fs[3] * Fs[3] * df.dx(meshes[3]))
        ) * param[
    ...
    if typ == "u":
        a = df.Constant(
            ...
            * df.assemble(Fs[1] * Fs[1] * df.dx(meshes[1]))
            * df.assemble(Fs[2] * Fs[2] * df.dx(meshes[2]))
        ) * param["k"] * fct_F * var_F * df.dx(meshes[3]) + df.Constant(
            df.assemble(Fs[0] * Fs[0] * df.dx(meshes[0]))
            * df.assemble(Fs[1].dx(0) * Fs[1].dx(0) * df.dx(meshes[1]))
            * df.assemble(Fs[2] * Fs[2] * df.dx(meshes[2]))
        ) * param[
...
def problem_assemble_rhs_FEM(
    ...
    if typ == "r":
        l = (
            ...
            * var_F
            * df.dx(meshes[0])
        )  # T*Q - (k*dT/dx*DTBC/dx + k*dT/dx*DTBC/dx)
        ...
        if nE > 0:
            for old in range(nE):
                l += -df.Constant(
                ...
                ) * param["k"] * PGD_func[0][old].dx(0) * var_F.dx(0) * df.dx(
                    meshes[0]
                ) - df.Constant(
                    ...
                    * df.assemble(PGD_func[3][old] * Fs[3] * df.dx(meshes[3]))
                    ...
                    meshes[0]
                )  # -k*dT/dx*DTold/dx - k*dT/dx*DTold/dx

    if typ == "s":
        l = (
            df.Constant(
                df.assemble(Q[0][0] * Fs[0] * df.dx(meshes[0]))
                * df.assemble(Q[2][0] * Fs[2] * df.dx(meshes[2]))
                * df.assemble(Q[3][0] * Fs[3] * df.dx(meshes[3]))
            )
            * Q[1][0]
            ...
            * df.dx(meshes[1])
            - df.Constant(
                df.assemble(BC_x.dx(0) * Fs[0].dx(0) * df.dx(meshes[0]))
                * df.assemble(BC_q * Fs[2] * df.dx(meshes[2]))
                * df.assemble(BC_u * Fs[3] * df.dx(meshes[3]))
            )
            * param["k"]
            ...
            * df.dx(meshes[1])
            - df.Constant(
                df.assemble(BC_x * Fs[0] * df.dx(meshes[0]))
                * df.assemble(BC_q * Fs[2] * df.dx(meshes[2]))
                * df.assemble(BC_u * Fs[3] * df.dx(meshes[3]))
            )
            * param["k"]
        ...
        if nE > 0:
            for old in range(nE):
                l += -df.Constant(
                    df.assemble(PGD_func[0][old].dx(0) * Fs[0].dx(0) * df.dx(meshes[0]))
                    * df.assemble(PGD_func[2][old] * Fs[2] * df.dx(meshes[2]))
                    * df.assemble(PGD_func[3][old] * Fs[3] * df.dx(meshes[3]))
                ) * param["k"] * PGD_func[1][old] * var_F * df.dx(
                    meshes[1]
                ) - df.Constant(
                    df.assemble(PGD_func[0][old] * Fs[0] * df.dx(meshes[0]))
                    * df.assemble(PGD_func[2][old] * Fs[2] * df.dx(meshes[2]))
                    * df.assemble(PGD_func[3][old] * Fs[3] * df.dx(meshes[3]))
                ) * param[
                    "k"
        ...
        if nE > 0:
            for old in range(nE):
                l += -df.Constant(
                    ...
                    * df.assemble(PGD_func[1][old] * Fs[1] * df.dx(meshes[1]))
                    * df.assemble(PGD_func[3][old] * Fs[3] * df.dx(meshes[3]))
                ) * param["k"] * PGD_func[2][old] * var_F * df.dx(
                    meshes[2]
                ) - df.Constant(
                    df.assemble(PGD_func[0][old] * Fs[0] * df.dx(meshes[0]))
                    * df.assemble(
                        PGD_func[1][old].dx(0) * Fs[1].dx(0) * df.dx(meshes[1])
                    )
                    * df.assemble(PGD_func[3][old] * Fs[3] * df.dx(meshes[3]))
                ) * param[
                    "k"
        ...
        if nE > 0:
            for old in range(nE):
                l += -df.Constant(
                    ...
                    * df.assemble(PGD_func[1][old] * Fs[1] * df.dx(meshes[1]))
                    * df.assemble(PGD_func[2][old] * Fs[2] * df.dx(meshes[2]))
                ) * param["k"] * PGD_func[3][old] * var_F * df.dx(
                    meshes[3]
                ) - df.Constant(
                    df.assemble(PGD_func[0][old] * Fs[0] * df.dx(meshes[0]))
                    * df.assemble(
...
def problem_assemble_lhs_FD(fct_F, var_F, Fs, meshes, dom, param, typ, dim):
    ...
    if typ == "r":
        a = (
            ...
            @ param["M_y"]
            @ Fs[1].vector()[:]
            * Fs[2].vector()[:].transpose()
            @ param["M_q"]
            @ Fs[2].vector()[:]
            * Fs[3].vector()[:].transpose()
            @ param["M_u"]
            @ Fs[3].vector()[:]
            * param["k"]
            * param["D2_x"]
...
def problem_assemble_rhs_FD(
    ...
    if typ == "r":
        l = (
            Fs[1].vector()[:].transpose()
            @ param["M_y"]
            @ Q[1][0].vector()[:].transpose()
            * Fs[2].vector()[:].transpose()
            @ param["M_q"]
            @ Q[2][0].vector()[:].transpose()
            * Fs[3].vector()[:].transpose()
            @ param["M_u"]
            @ Q[3][0].vector()[:].transpose()
            * param["M_x"]
            @ Q[0][0].vector()[:]
            + Fs[1].vector()[:].transpose()
            @ param["M_y"]
        ...
        if nE > 0:
            for old in range(nE):
                l += (
                    ...
                    * Fs[2].vector()[:].transpose()
                    @ param["M_q"]
                    @ PGD_func[2][old].vector()[:]
                    * Fs[3].vector()[:].transpose()
                    @ param["M_u"]
                    @ PGD_func[3][old].vector()[:]
                    * param["k"]
                    * param["D2_x"]
                    @ PGD_func[0][old].vector()[:]
                    + Fs[1].vector()[:].transpose()
                    @ param["D2_y"]
                    @ PGD_func[1][old].vector()[:]
                    * Fs[2].vector()[:].transpose()
...
def create_PGD(param=[], vs=[], _type=None):
    ...
    qx = [
        df.interpolate(
            df.Expression("x[0]<L/2 ? 1.0 : 0", degree=1, L=param["lx"]), vs[0]
        )
    ]
    ...
    if _type == "FEM":
        ...
        param["M_x"], param["D2_x"] = (
            M_x[idx_sort[0], :][:, idx_sort[0]],
            D2_x[idx_sort[0], :][:, idx_sort[0]],
        )
        param["M_y"], param["D2_y"] = (
            M_y[idx_sort[1], :][:, idx_sort[1]],
            D2_y[idx_sort[1], :][:, idx_sort[1]],
        )
        param["M_q"], param["M_u"] = (
            M_q[idx_sort[2], :][:, idx_sort[2]],
            M_u[idx_sort[3], :][:, idx_sort[3]],
        )
        param["bc_idx"] = np.array(
            [np.where(x_dofs[0] == 0)[0], (np.where(x_dofs[0] == param["lx"])[0])]
        ).flatten()
...
class FEM_reference:
    def __init__(self, param=[], elem=[], ord=[]):
        ...
        self.mesh = df.RectangleMesh(
            df.Point(0, 0),
            df.Point(self.param["lx"], self.param["ly"]),
            self.elem[0],
            self.elem[1],
        )
        self.V = df.FunctionSpace(
            self.mesh, "CG", ord[0] + 1
        )  # second order breaked down 1.order in pgd split

        self.u00 = df.Constant(1.0)
        self.q00 = df.Constant(1.0)
        #
        self.bcExpr = df.Expression("u00*(1. - 1./3.*x[0])", degree=1, u00=self.u00)
        self.Q = df.Expression(
            "x[0]<L/2 ? q00 : 0", degree=1, L=self.param["lx"], q00=self.q00
        )
        #
        v = df.TestFunction(self.V)
        T = df.TrialFunction(self.V)
        self.a = (
            self.param["k"] * df.inner(df.grad(v), df.grad(T)) * df.dx()
        )  # SIGN????
    ...
    def __call__(self, values):
        ...
        def leftright(x, on_boundary):
            return (
                on_boundary
                and df.near(x[0], 0.0, 1e-6)
                or df.near(x[0], self.param["lx"], 1e-6)
            )
...
class problem(unittest.TestCase):
    ...
    def test_solver(self):
        ...
        print(
            "Number of Modes", pgd_fem.numModes, pgd_fd.numModes
        )  # should be converged in 1 mode set!!
        ...
        if self.plotting:
            ...
            ufem_x, fem_x, ufem_xy = FEM_reference(
                param=self.param, elem=self.elem, ord=self.ord
            )(self.values)
            ...
            upgd_fem_bc = upgd_fem.compute_vertex_values()[:] + param[
                "BC_x"
            ].compute_vertex_values()[:] * param["BC_y"](self.values[0]) * param[
                "BC_q"
            ](
                self.values[1]
            ) * param[
            ...
            plt.plot(
                upgd_fem.function_space().mesh().coordinates()[:],
                upgd_fem_bc,
                "-*r",
        ...
        for _ in range(10):
            check_values = [
                self.ranges[1][0]
                + np.random.random() * (self.ranges[1][1] - self.ranges[1][0]),
                self.ranges[2][0]
                + np.random.random() * (self.ranges[2][1] - self.ranges[2][0]),
                self.ranges[3][0]
                + np.random.random() * (self.ranges[3][1] - self.ranges[3][0]),
            ]
            u1 = pgd_fd.evaluate(
                self.fixed_dim, [1, 2, 3], check_values, 0
            ).compute_vertex_values()[:] + param["BC_x"].compute_vertex_values()[
                :
            ] * param[
                "BC_y"
            ](
            ...
            u3, _, _ = FEM_reference(param=self.param, elem=self.elem, ord=self.ord)(
                check_values
            )

            errors_FEM1.append(
                np.linalg.norm(u1 - u3) / np.linalg.norm(u3)
            )  # PGD FD - FEM
            errors_FEM2.append(
                np.linalg.norm(u2 - u3) / np.linalg.norm(u3)
            )  # PGD FEM - FEM
