...
def create_doms_pgd(Vs, param):
    ...
    class zero(dolfin.SubDomain):
        def inside(self, x, on_boundary):
            return on_boundary and dolfin.near(x, 0., dolfin.DOLFIN_EPS)

    class ly(dolfin.SubDomain):
        def inside(self, x, on_boundary):
            return on_boundary and np.isclose(x, np.max(Vs[1].mesh().coordinates()[:]), dolfin.DOLFIN_EPS)

    class lx(dolfin.SubDomain):
        def inside(self, x, on_boundary):
            return on_boundary and np.isclose(x, np.max(Vs[0].mesh().coordinates()[:]), dolfin.DOLFIN_EPS)
...
class problem(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.param = {"rho": 7100., "cp": 3100., "k": 100., 'Tamb': 25., 'T_ic': 500.,
                      ...
                      'L_0': 1, 't_0': 1., 'T_0': 1.}
                      #'L_0': 0.1, 't_0': 100., 'T_0': 500. }  # possibility to make the equation dimless
