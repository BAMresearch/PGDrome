...
def create_bc(Vs, dom, param):
    ...
    bc_l = dolfin.DirichletBC(Vs[0], 0.0, left)
    bc_r = dolfin.DirichletBC(Vs[0], 0.0, right)
    bcs = [bc_l, bc_r]
...
def problem_assemble_rhs(
    ...
    if typ == "s":
        l = (
            dolfin.Constant(
                dolfin.assemble(Fs[0] * G[0][0] * param["A"] * dolfin.dx(meshes[0]))
                * dolfin.assemble(Fs[2] * G[2][0] * dolfin.dx(meshes[2]))
            )
            * var_F
            * G[1][0]
            ...
            for old in range(nE):
                l += (
                    -dolfin.Constant(
                        ...
                        * dolfin.assemble(
                            Fs[1] * PGD_func[1][old] * dolfin.dx(meshes[1])
                        )
                    )
                    * var_F
                    * param["Efunc"]
...
class PGDproblem(unittest.TestCase):
    ...
    def test_standard_solver(self):
        ...
        # define meshes
        meshes, vs = create_meshes(
            [113, 2, 100], self.ords, self.ranges
        )  # start meshes
        ...
        data_test = [[2.0, 1.5], [1.0, 1.0]]  # [Amplitude, Elastic modulus]
